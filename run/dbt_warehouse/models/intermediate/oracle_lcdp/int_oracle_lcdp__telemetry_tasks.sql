...
select * from telemetry_tasks
    ...
    when matched then update set
        `task_product_id` = DBT_INTERNAL_SOURCE.`task_product_id`,`task_id` = DBT_INTERNAL_SOURCE.`task_id`,`device_id` = DBT_INTERNAL_SOURCE.`device_id`,`company_id` = DBT_INTERNAL_SOURCE.`company_id`,`product_id` = DBT_INTERNAL_SOURCE.`product_id`,`location_id` = DBT_INTERNAL_SOURCE.`location_id`,`device_code` = DBT_INTERNAL_SOURCE.`device_code`,`product_code` = DBT_INTERNAL_SOURCE.`product_code`,`company_code` = DBT_INTERNAL_SOURCE.`company_code`,`company_name` = DBT_INTERNAL_SOURCE.`company_name`,`task_location_info` = DBT_INTERNAL_SOURCE.`task_location_info`,`telemetry_source` = DBT_INTERNAL_SOURCE.`telemetry_source`,`telemetry_send_cause` = DBT_INTERNAL_SOURCE.`telemetry_send_cause`,`task_start_date` = DBT_INTERNAL_SOURCE.`task_start_date`,`telemetry_quantity` = DBT_INTERNAL_SOURCE.`telemetry_quantity`,`sale_unit_price_eur` = DBT_INTERNAL_SOURCE.`sale_unit_price_eur`,`sale_amount_net_eur` = DBT_INTERNAL_SOURCE.`sale_amount_net_eur`,`sale_amount_net_tax_eur` = DBT_INTERNAL_SOURCE.`sale_amount_net_tax_eur`,`updated_at` = DBT_INTERNAL_SOURCE.`updated_at`,`created_at` = DBT_INTERNAL_SOURCE.`created_at`,`extracted_at` = DBT_INTERNAL_SOURCE.`extracted_at`
    

    when not matched then insert
        (`task_product_id`, `task_id`, `device_id`, `company_id`, `product_id`, `location_id`, `device_code`, `product_code`, `company_code`, `company_name`, `task_location_info`, `telemetry_source`, `telemetry_send_cause`, `task_start_date`, `telemetry_quantity`, `sale_unit_price_eur`, `sale_amount_net_eur`, `sale_amount_net_tax_eur`, `updated_at`, `created_at`, `extracted_at`)
    values
        (`task_product_id`, `task_id`, `device_id`, `company_id`, `product_id`, `location_id`, `device_code`, `product_code`, `company_code`, `company_name`, `task_location_info`, `telemetry_source`, `telemetry_send_cause`, `task_start_date`, `telemetry_quantity`, `sale_unit_price_eur`, `sale_amount_net_eur`, `sale_amount_net_tax_eur`, `updated_at`, `created_at`, `extracted_at`)

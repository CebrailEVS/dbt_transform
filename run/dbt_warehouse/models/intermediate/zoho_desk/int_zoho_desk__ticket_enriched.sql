    ...
    OPTIONS(
      description="""Vue enrichie 1:1 par ticket \u2014 fondation des marts ticket (dim). Joint tickets + ticket_details (custom fields) + ticket_metrics (SLA, compteurs) + accounts (avec fallback via le contact quand t.account_id est NULL) + contacts + departments + agents. Chaque ligne contient tout ce dont le m\u00e9tier a besoin pour analyser un ticket : compte client, contact, agent assign\u00e9, d\u00e9partement, cat\u00e9gories, nature, threads, r\u00e9ponses agent, dur\u00e9es SLA.\n"""
    )
    as (

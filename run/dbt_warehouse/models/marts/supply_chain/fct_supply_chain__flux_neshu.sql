    ...
    create or replace table `evs-datastack-prod`.`prod_marts`.`fct_supply_chain__flux_neshu`
      
    partition by mois_date
    

    
    OPTIONS(
      description="""Table de faits mensuelle consolidant l'ensemble des flux supply Neshu. Elle combine : - Les stocks r\u00e9els issus des inventaires (prioritaires) - Les stocks th\u00e9oriques (fallback si absence d'inventaire) - Les flux op\u00e9rationnels (r\u00e9ceptions, livraisons, chargements) La granularit\u00e9 est mensuelle (mois_date).\n"""
    )
    as (
...
with inventaire_base as (
    ...
    left join `evs-datastack-prod`.`prod_marts`.`dim_neshu__resource` as r on i.source_code = r.resources_code
    where
...
stock_theorique as (
    ...
    left join `evs-datastack-prod`.`prod_marts`.`dim_neshu__product` as p on st.product_code = p.product_code
    left join
        inventaire_sources_mois as ir
        on st.resources_code = ir.source_code and ir.mois = date_trunc(date(st.date_system), month)
    left join `evs-datastack-prod`.`prod_marts`.`dim_neshu__resource` as rr on st.resources_code = rr.resources_code
...
select
    ...
    'dee3c756-046b-46db-822e-5791da48e93a' as dbt_invocation_id
from agg
    );

    ...
    create or replace table `evs-datastack-prod`.`prod_staging`.`stg_yuman_evs_sftp__stock_theorique`
    ...
    OPTIONS(
      description="""Snapshot quotidien du stock th\u00e9orique Yuman, nettoy\u00e9 et typ\u00e9. Une ligne par (r\u00e9f\u00e9rence, emplacement) pr\u00e9sente dans le fichier du jour ; l'absence de ligne vaut quantit\u00e9 nulle, Yuman ne conservant pas les emplacements vides. Remplace stg_yuman_gcs__stock_theorique, dont la sortie m\u00e9tier est identique.\n"""
    )
    as (
...
with source as (
    ...
    select *
    from `evs-datastack-prod`.`prod_raw`.`sftp_yuman_evs_stock_theorique`

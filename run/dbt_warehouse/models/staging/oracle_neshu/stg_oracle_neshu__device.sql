    ...
    create or replace table `evs-datastack-prod`.`prod_staging`.`stg_oracle_neshu__device`
      
    
    cluster by iddevice, idlocation, idcompany_customer

    
    OPTIONS(
      description="""Devices transform\u00e9s et nettoy\u00e9s depuis la base Oracle"""
    )
    as (

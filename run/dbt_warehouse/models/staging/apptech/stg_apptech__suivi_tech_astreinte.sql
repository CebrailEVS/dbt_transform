    ...
    create or replace table `evs-datastack-prod`.`prod_staging`.`stg_apptech__suivi_tech_astreinte`
    ...
    OPTIONS(
      description="""R\u00e9affectations d'astreinte : technicien r\u00e9el ayant effectu\u00e9 l'intervention. Dedup snapshot : seules les lignes du dernier fichier NDJSON de chaque mois sont conserv\u00e9es. Pas de test de freshness (soumissions humaines).\n"""
    )
    as (
...
with source_data as (
    ...
    from `evs-datastack-prod`.`prod_raw`.`ext_gcs_apptech__suivi_tech_astreinte`

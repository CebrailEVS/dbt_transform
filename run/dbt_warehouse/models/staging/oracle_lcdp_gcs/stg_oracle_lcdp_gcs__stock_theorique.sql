    ...
    create or replace table `evs-datastack-prod`.`prod_staging`.`stg_oracle_lcdp_gcs__stock_theorique`
    ...
    OPTIONS(
      description="""Stock th\u00e9orique LCDP, une photo par jour et par couple entit\u00e9/article. Charg\u00e9 par le pipeline dlt `oracle_lcdp_stock` (d\u00e9p\u00f4t ingestion) depuis Oracle. Les colonnes sont typ\u00e9es, et les deux horloges Oracle \u2014 nues c\u00f4t\u00e9 source \u2014 sont d\u00e9clar\u00e9es en Europe/Paris.\n"""
    )
    as (
...
select
    ...
    snapshot_date
from `evs-datastack-prod`.`prod_raw`.`oracle_lcdp_stock_theorique`
    );

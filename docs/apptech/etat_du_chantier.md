...
| ~~Seed `ref_nesp_tech__key_facturation` versionné `valid_from`/`valid_to` + 2 variantes Montagne manquantes~~ | DE | — | ✅ **PR #202 mergée** 2026-08-07 (`39d8256e`), `cd` vert, prod vérifiée (138 lignes versionnées, 4 tarifs réparés) |
...
| ~~Mart des avoirs Repair Warranty~~ → livré sous le nom `fct_technique__credit_repair_warranty` (**marts**, pas intermediate : cf. § 4) | DE | — | ✅ fait 2026-08-07, branche `feature/credit-repair-warranty` (`88da7983`) |
| **Supprimer du bucket les 2 NDJSON de test d'ingestion** `RW_TEST_DATA_ING_2026-01/07.xlsx` (vraies fautives, correctives factices) — le mart les filtre, mais la source devrait être propre | **DA** | — | à demander |
...
| **Le calcul des avoirs RW va en marts, pas en intermediate** (`fct_technique__credit_repair_warranty`). Deux raisons : le modèle croise apptech × nesp_tech, ce que `docs/conventions/intermediate.md:13` interdit ; et c'est un fait au sens propre — un événement daté porteur d'une mesure, référençant des dimensions. L'étape de compilation source-alignée qui justifierait un intermediate n'existe pas ici, le flux RW étant déjà normalisé au staging | 2026-08-07 | arbitré avec le user |

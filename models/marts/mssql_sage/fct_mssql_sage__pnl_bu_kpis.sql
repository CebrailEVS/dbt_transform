...
and (s.scenario='AVEC_PROVISIONS_CP' or (s.scenario='SANS_PROVISIONS_CP' and f.numero_compte_general not in ('645800','641200')))
),

kpi_mensuel as (
-- KPI mensuels réel
select
scenario,annee,mois,bu,
sum(case when macro_categorie_pnl_bu="Chiffre d'Affaires" then montant_analytique_signe else 0 end) as chiffre_affaire,
sum(case when macro_categorie_pnl_bu='Consommation MP & SSTT' then montant_analytique_signe else 0 end) as consommation_mp_sstt,
sum(case when macro_categorie_pnl_bu='Masse Salariale' then montant_analytique_signe else 0 end) as masse_salariale,
sum(case when macro_categorie_pnl_bu='Frais Directs & Amortissements' then montant_analytique_signe else 0 end) as frais_directs_amortissements
from base
...
-- Format long réel
select
scenario,annee,mois,bu,kpi,valeur,valeur_ytd,valeur_n_1,valeur_ytd_n_1
from kpi_enrichi
...
from {{ ref('ref_mssql_sage__pnl_budget') }}
),
...
sum(frais_directs_amortissements) over(partition by annee,bu order by mois) as frais_directs_amortissements_ytd,
...
-- Format long budget
select
annee,mois,bu,kpi,valeur,valeur_ytd
from budget_kpi_ytd
...
from kpi_long l
left join kpi_long ca on l.scenario=ca.scenario and l.annee=ca.annee and l.mois=ca.mois and l.bu=ca.bu and ca.kpi='CA'
left join budget_long b on l.annee=b.annee and l.mois=b.mois and l.bu=b.bu and l.kpi=b.kpi
)

...
WITH company_labels AS (
  SELECT
    ...
    loc.country
  FROM {{ ref('stg_oracle_lcdp__company') }} c
  LEFT JOIN {{ ref('stg_oracle_lcdp__label_has_company') }} lhc
  ...
  LEFT JOIN {{ ref('stg_oracle_lcdp__company_has_location') }} chl
    ON chl.idcompany = c.idcompany AND chl.idlocation_type = 1
  LEFT JOIN {{ ref('stg_oracle_lcdp__location') }} loc

{{
    config(
        ...
        cluster_by=['device_id', 'product_id', 'resources_roadman_id']
    )
}}
...
left join {{ ref('dim_lcdp__device') }} as d on m.device_id = d.device_id
left join {{ ref('dim_lcdp__product') }} as p on m.product_id = p.product_id
left join {{ ref('dim_lcdp__resource') }} as r on m.roadman_id = r.resources_id

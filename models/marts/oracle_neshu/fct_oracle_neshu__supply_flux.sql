...
-- =========================================================
-- CTE : inventaire_base
-- Inventaires valides + filtre ressources actives + normalisation DATE
-- =========================================================
...
from {{ ref('int_oracle_neshu__inventaire_tasks') }} i
left join {{ ref('dim_oracle_neshu__resources') }} r on i.source_code=r.resources_code
where i.task_status_code='VALIDE'
and date(i.task_start_date)>='{{ start_date }}'
and (r.is_active=true or r.is_active is null)
),

-- =========================================================
-- CTE : inventaire_dates
-- Calcul :
-- - dernière date du mois
-- - fallback : première date <=5 du mois suivant
-- Optimisation : limitation du scan via fenêtre temporelle
-- =========================================================
inventaire_dates as (
select
b.source_code,
...
array_agg(
case
when date_trunc(b2.task_date,month)=date_add(b.mois,interval 1 month)
...
from inventaire_base b
left join inventaire_base b2
on b.source_code=b2.source_code
and b2.task_date between b.mois and date_add(b.mois,interval 40 day)
group by b.source_code,b.mois
),

-- =========================================================
-- CTE : inventaire_reference
-- Choix de la date de référence avec fallback
-- =========================================================
...
-- =========================================================
-- CTE : inventaire_reel
-- Extraction des inventaires retenus
-- =========================================================
inventaire_reel as (
select
r.mois,
...
i.product_code,
i.valuation
from inventaire_reference r
join inventaire_base i
on r.source_code=i.source_code
and r.ref_date=i.task_date
),

-- =========================================================
-- CTE : inventaire_sources_mois
-- Sources ayant un inventaire réel (exclusion fallback)
-- =========================================================
...
-- =========================================================
-- CTE : stock_theorique
-- Fallback uniquement si pas d'inventaire réel
-- =========================================================
stock_theorique as (
select
...
left join {{ ref('dim_oracle_neshu__product') }} p on st.product_code=p.product_code
left join inventaire_sources_mois ir
on ir.source_code=st.resources_code
and ir.mois=date_trunc(date(st.date_system),month)
left join {{ ref('dim_oracle_neshu__resources') }} rr on rr.resources_code=st.resources_code
...
-- =========================================================
-- CTE : flux_unifies
-- Normalisation de tous les flux supply
-- =========================================================
flux_unifies as (
select
mois as date,
...
when destination_code like 'V%' or destination_code in ('RATP','ARKEMA','STRASBOURG') then 'LIVRAISON_VEHICULE'
...
and date(task_start_date) between '{{ flux_start }}' and '{{ flux_end }}'
union all
select
date(task_start_date),
valuation,
...
where task_status_code in ('FAIT','VALIDE')
and destination_code not in ('DEPOTATELIERBORDEAUX','DEPOTATELIERLYON','DEPOTATELIERMARSEILLE','DEPOTATELIERRUNGIS')
and date(task_start_date) between '{{ flux_start }}' and '{{ flux_end }}'
union all
select
date(task_start_date),
...
where task_status_code in ('VALIDE','FAIT')
and date(task_start_date) between '{{ flux_start }}' and '{{ flux_end }}'
union all
select
date(task_start_date),
...
where date(task_start_date) between '{{ flux_start }}' and '{{ flux_end }}'
),
...
sum(if(flux_type in ('STOCK_DEPOT_RUNGIS','STOCK_DEPOT_LYON','STOCK_DEPOT_MARSEILLE'),valeur,0)) as stock_depot,

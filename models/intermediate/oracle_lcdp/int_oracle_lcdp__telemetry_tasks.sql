...
with telemetry_tasks as (
    ...
    left join {{ ref('stg_oracle_lcdp__device') }} d
        on d.iddevice = t.iddevice

    left join {{ ref('stg_oracle_lcdp__location') }} l
        on l.idlocation = t.idlocation

    -- Jointures pour le filtrage sur labels télémétrie
    inner join {{ ref('stg_oracle_lcdp__label_has_task') }} lht
        on t.idtask = lht.idtask

    inner join {{ ref('stg_oracle_lcdp__label') }} la
        on lht.idlabel = la.idlabel

    inner join {{ ref('stg_oracle_lcdp__label_family') }} lf

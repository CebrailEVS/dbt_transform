...
resources_roadman AS (
    ...
    JOIN {{ ref('stg_oracle_lcdp__resources') }} r
...
pointage AS (
    ...
    JOIN {{ ref('stg_oracle_lcdp__label_has_task') }} lht ON t.idtask = lht.idtask
    JOIN {{ ref('stg_oracle_lcdp__label') }} la ON lht.idlabel = la.idlabel AND la.idlabel in (2685,2686)  -- START_DAY & END_DAY uniquement
